...
const benchmarks = {
  intelligence: {
    ...
    noteZh: 'OAB 与 VoiceBench 总分取各子项得分平均值，不代表官方完整得分；报告中的 MultiChallenge 以 benchmark 为计分单位。'
  },
  action: {
    ...
    noteZh: 'WebSearch1K 为自研的 S2T 时效性评测基准。相较于 3.0 版本，平均查询次数减少 76.0%，F1 分数下降 2.26 个百分点；查询次数的减少，并不直接代表回答质量、实测延迟或成本的改善。'
  },
  empathy: {
    ...
    noteZh: '所有评测均采用 1–5 分制。CharacterEval 与 RMTBench 采用公开数据集子集，并基于口语角色扮演 rubric 重新评分；PersonaCross、20+ 轮对话及中文真实语音共情均为自研评测。共情能力评测采用语音输入、文本输出模式，不涉及语音韵律生成。'
  },
  reliability: {
    ...
    noteZh: '多轮攻击与能力边界为自研评测，攻击成功率越低越好。此项自动化测试指标无法衡量模型对无害请求的拒绝情况、人工红队测试结果或运行时安全策略。'
  },
  multilingual: {
    ...
    noteZh: 'ASR 与 MultiASR 的指标为词错误率（越低越好），QA 与 BLEU 分数则越高越好。Qwen-Audio-3.0/3.1-Realtime 与 Gemini-3.7-Flash 的 FunAU 结果取自内部 benchboard，GPT-Realtime-2 与 SeedDuplex 未参与该数据集评测。评测所用语言不代表模型支持的全部语种。'
  },
  context: {
    ...
    noteZh: 'Long-AMC 为 Audio MultiChallenge 中面向长上下文场景的评测子集。在报告所述评测条件下，三项指标均有提升。'
  }
};
