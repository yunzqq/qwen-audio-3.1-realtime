...
function renderBenchmarks(category) {
  ...
  charts.innerHTML = data.metrics.map(m => {
    const top = axisTop(m);
    return `<article class="chart-card"><h3>${m.name}</h3><p class="chart-subtitle">${m.lower ? '↓ Lower is better' : '↑ Higher is better'} · ${m.unit === '/5' ? '1–5 score' : m.unit}</p><div class="col-plot"><div class="col-axis" aria-hidden="true"><span>${top}</span><span>${top / 2}</span><span>0</span></div><div class="col-set">${m.values.map((value, index) => {
...
document.querySelector('.table-toggle').addEventListener('click', event => {
  ...
  event.currentTarget.textContent = showTable ? 'Chart view' : 'Table view';
