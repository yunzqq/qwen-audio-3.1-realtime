...
const benchmarks = {
  intelligence: {
    ...
    noteZh: 'OAB 与 VoiceBench 总分系各子项得分的平均值，不代表官方完整评分；报告中的 MultiChallenge 均以 benchmark 为计分单位。',
  },
  action: {
    ...
    noteZh: 'WebSearch1K 系自研的 S2T 时效性基准测试。较 3.0 版本，其平均查询次数减少 76.0%，F1 分数下降 2.26 个百分点；查询次数的降低，并不直接等同于回答质量、实测延迟或成本的实质优化。',
  },
  empathy: {
    ...
    noteZh: '所有评测均采用 1–5 分制。CharacterEval 与 RMTBench 选取公开数据集的子集，并基于口语角色扮演 rubric 重新评分；PersonaCross、20+ 轮对话及中文真实语音共情均为自研评测。共情能力评测采用语音输入、文本输出模式，不涉及语音韵律生成。',
  },
  reliability: {
    ...
    noteZh: '多轮攻击与能力边界均为自研评测，攻击成功率以低为优。该自动化测试指标无法反映模型对无害请求的拒绝情况、人工红队测试结果或运行时安全策略。',
  },
  multilingual: {
    ...
    noteZh: 'ASR 与 MultiASR 的衡量指标为词错误率（以低为优），QA 与 BLEU 分数则以高为优。Qwen-Audio-3.0/3.1-Realtime 与 Gemini-3.7-Flash 的 FunAU 结果源自内部 benchboard，GPT-Realtime-2 与 SeedDuplex 未参与该数据集评测。评测所用语言不代表模型支持的全部语种。',
  },
  context: {
    ...
    noteZh: 'Long-AMC 为 Audio MultiChallenge 中针对长上下文场景的评测子集。在报告所述的评测条件下，三项指标均实现提升。',
  }
};
...
function renderBenchmarks(category) {
  ...
  table.innerHTML = `<table><caption>${T(`Exact values · ${category} · — indicates an unavailable result`, `精确数值 · ${category} · — 表示缺失结果`)}</caption><thead><tr><th scope="col">Model</th>${data.metrics.map(m => `<th scope="col">${m.name} ${m.lower ? '↓' : '↑'}</th>`).join('')}</tr></thead><tbody>${modelNames.map((name, index) => `<tr class="${index === 3 ? 'current' : ''}"><th scope="row">${name}</th>${data.metrics.map(m => `<td>${m.values[index] === null ? '—' : m.values[index].toFixed(2)}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
...
const TECH_FIGURES = {
  ...
  posttrain: {title: 'Post-Training', src: 'assets/tech-post-training.png?v=2'},
  selfevolve: {title: 'Self-Evolving Environment', src: 'assets/tech-self-evolving.png?v=2'},
